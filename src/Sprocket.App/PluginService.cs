...
internal static class PluginService
{
    ...
    private static IReadOnlyList<string> WithUserFolder(IReadOnlyList<string> systemDirectories, string formatFolder)
    {
        ...
        if (!string.IsNullOrEmpty(appData))
            dirs.Add(Path.Combine(appData, "DaVinci Project", "Plugins", formatFolder));
    ...
    private static IReadOnlyList<PluginManager.PluginDirectory> PluginDirectories()
    {
        ...
        string appData = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
        if (!string.IsNullOrEmpty(appData))
            directories.Add(new(Path.Combine(appData, "DaVinci Project", "Plugins"), IsUserWritable: true));
        return directories;
    }
}
