...
internal static class Doctor
{
    ...
    private static bool CheckLinuxLibc()
    {
        ...
        try
        {
            ...
            if (v < GlibcBaseline)
                Warn($"libc    : glibc {v} is BELOW the supported baseline {GlibcBaseline} (Ubuntu 22.04) — DaVinci Project may run but is unsupported here.");
            else
